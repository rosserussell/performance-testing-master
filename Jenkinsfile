  ...
  stages {
   ...
   stage('Get Exploratory Testing Repo')
   {
      steps {
        ...
        git(
        url:'git@github.com:rosserussell/performance-testing-master.git',
        credentialsId: 'Performance_Test',
        branch: "master"
        )
     }
        ...
        steps
        {
           ...
           dir('./performance-test')
           {
              ...
              script {
                 ...
                 sh 'ls -l'
                 sh '/usr/local/bin/apache-jmeter-5.2/bin/jmeter.sh -n -t ./test-plan.jmx -l ./test-results.csv'
                 sh 'mkdir html-report'
                 sh 'ls -l'
                 echo "create html report"
                 sh '/usr/local/bin/apache-jmeter-5.2/bin/jmeter.sh -g ./test-results.csv -e -o html-report'
